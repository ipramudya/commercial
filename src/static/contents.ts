import {
    MaterialFiveImg,
    MaterialFourImg,
    MaterialOneImg,
    MaterialThreeImg,
    MaterialTwoImg,
    WorksDetail10Img,
    WorksDetail11Img,
    WorksDetail12Img,
    WorksDetail13Img,
    WorksDetail14Img,
    WorksDetail15Img,
    WorksDetail16Img,
    WorksDetail17Img,
    WorksDetail18Img,
    WorksDetail1Img,
    WorksDetail2Img,
    WorksDetail3Img,
    WorksDetail4Img,
    WorksDetail5Img,
    WorksDetail6Img,
    WorksDetail7Img,
    WorksDetail8Img,
    WorksDetail9Img,
    WorksFiveImg,
    WorksFourImg,
    WorksOneImg,
    WorksSevenImg,
    WorksSixImg,
    WorksThreeImg,
    WorksTwoImg,
} from "@/assets/images";

export const WeenIndokraftTeks = "Ween Indokraft";
export const WeenIndokraftMeaningTeks =
    "adalah perusahaan supplier furnitur yang telah berdiri sejak tahun 2024. Perusahaan ini telah berpengalaman dalam menyediakan berbagai jenis produk furnitur, termasuk meja, kursi, lemari, dan furnitur lainnya, kepada pelanggan atau bisnis. Dengan reputasi yang terjamin, Ween Indokraftt menawarkan kualitas yang terjamin, harga yang bersaing, dan kemudahan dalam penyediaan furnitur.";
export const SupplyTeks =
    "Ween indokraft bersedia melakukan penyediaan atau penyaluran furnitur kepada pelanggan atau pengguna. Dalam konteks bisnis, mensupply furnitur melibatkan perusahaan atau individu yang menyediakan berbagai jenis furnitur, seperti meja, kursi, lemari, dan lain-lain, untuk memenuhi kebutuhan konsumen";
export const CustomTeks =
    "Ween indokraft dapat melakukan kustom furnitur atau barang yang dibuat sesuai dengan pesanan atau kebutuhan individu. Dalam konteks ini, kustom furniture dibuat dengan model dan ukuran yang disesuaikan untuk memenuhi kebutuhan khusus dari pelanggan.";
export const OfficeTeks1 = "Jl. Bintaro Raya nomor 8C Kebayoran Lama, Jakarta Selatan DKI Jakarta";
export const OfficeTeks2 = "Ruko Shibuya Blok E no.9 PIK 2, Jakarta";
export const WorkshopTeks = "Jl. Kapuk Pulo Raya Kav.115 Jakarta Barat, DKI Jakarta";
export const WhatsappTeks = "+62819 1888 1686";
export const WhatsappNumTeks = "6281918881686";

export const MaterialsContents = [
    { id: 1, src: MaterialOneImg, name: "HPL" },
    { id: 2, src: MaterialTwoImg, name: "Triplek" },
    { id: 3, src: MaterialThreeImg, name: "Stainless Gold Mirror" },
    { id: 4, src: MaterialFourImg, name: "Stainless Black Mirror" },
    { id: 5, src: MaterialFiveImg, name: "Glass Door" },
];

export const WorksContents = [
    { id: 1, src: WorksOneImg },
    { id: 2, src: WorksTwoImg },
    { id: 3, src: WorksThreeImg },
    { id: 4, src: WorksFourImg },
    { id: 5, src: WorksFiveImg },
    { id: 6, src: WorksSixImg },
    { id: 7, src: WorksSevenImg },
];

export const WorksDetailContents = [
    { id: 1, src: WorksDetail1Img },
    { id: 2, src: WorksDetail2Img },
    { id: 3, src: WorksDetail3Img },
    { id: 4, src: WorksDetail4Img },
    { id: 5, src: WorksDetail5Img },
    { id: 6, src: WorksDetail6Img },
    { id: 7, src: WorksDetail7Img },
    { id: 8, src: WorksDetail8Img },
    { id: 9, src: WorksDetail9Img },
    { id: 10, src: WorksDetail10Img },
    { id: 11, src: WorksDetail11Img },
    { id: 12, src: WorksDetail12Img },
    { id: 13, src: WorksDetail13Img },
    { id: 14, src: WorksDetail14Img },
    { id: 15, src: WorksDetail15Img },
    { id: 16, src: WorksDetail16Img },
    { id: 17, src: WorksDetail17Img },
    { id: 18, src: WorksDetail18Img },
];
